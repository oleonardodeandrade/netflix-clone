...
import type { WatchProgress } from '../../services'
import { favoriteMoviesAtom } from '../../store/movies'
import { useToggleFavorite } from '../../hooks/useToggleFavorite'
import { watchProgressStorage } from '../../services'
...
export const ContinueWatchingCard = memo(function ContinueWatchingCard({
  ...
  const { movie } = watchProgress
  const isFavorite = favorites.some((fav) => fav.id === movie.id)
  const progressPercentage = (watchProgress.progress / watchProgress.duration) * 100
  ...
  const formatTimeRemaining = () => {
    const remaining = watchProgress.duration - watchProgress.progress
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-zinc-900 via-zinc-900/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300">
          <div className="absolute bottom-0 left-0 right-0 p-3 space-y-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300 delay-100">
            ...
            <div className="flex items-center gap-2 text-xs">
              ...
              <span className="text-white/70">{formatTimeRemaining()}</span>
            </div>
